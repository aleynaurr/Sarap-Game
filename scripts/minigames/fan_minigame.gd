...
func _unhandled_input(event: InputEvent) -> void:
	...
	var input_pressed = -1
	if event.is_action_pressed("move_up_p%d" % player_number) or (event is InputEventKey and event.keycode == KEY_W):
		input_pressed = 0
	elif event.is_action_pressed("move_down_p%d" % player_number) or (event is InputEventKey and event.keycode == KEY_S):
		input_pressed = 2
