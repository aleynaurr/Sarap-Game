...
func _on_init() -> void:
	...
	bowl_bg.position = Vector2(220, 220)
	add_child(bowl_bg)

	_bowl_label = make_label("🥣", 52, Color(1, 1, 1))
	_bowl_label.position = Vector2(275, 215)
	...
	track_bg.position = Vector2(110, 155)
	add_child(track_bg)
	...
	pz.position = Vector2(110 + int(420 * (0.5 - PERFECT_ZONE)), 155)
	add_child(pz)

	var pz_lbl = make_label("PERFECT", 9, Color(0.2, 1.0, 0.3))
	pz_lbl.position = Vector2(pz.position.x + 4, 187)
	add_child(pz_lbl)
	...
	gz.position = Vector2(110 + int(420 * (0.5 - GOOD_ZONE)), 155)
	add_child(gz)
	...
	_result_label.position = Vector2(200, 450)
