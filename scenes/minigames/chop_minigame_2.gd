...
@onready var _bowls: Array[TextureRect] = [
	$BowlRow/BowlBatwan,
	$BowlRow/BowlJackfruit,
	$BowlRow/BowlOnion
]
@onready var _icons: Array[TextureRect] = [
	$TopRow/IconBatwan,
	$TopRow/IconJackfruit,
	$TopRow/IconOnion
]
...
func _on_init() -> void:
	...
	for veg in VEGGIES:
		...
		for f in range(frames):
			veg_frames.append(load("res://assets/dish2_assets_chop/board_%s_%02d.png" % [vid, f]))
		_board_frames.append(veg_frames)

		_bowl_tex.append([
			load("res://assets/dish2_assets_chop/bowl_%s_empty.png" % vid),
			load("res://assets/dish2_assets_chop/bowl_%s_filled.png" % vid),
		])
...
func _do_chop() -> void:
	if _done: return
	print("CHOP called | veg_idx=", _veg_idx, " frame(before)=", _frame, " cuts_needed=", VEGGIES[_veg_idx][2] - 1)
	_hide_start_popup_now()
